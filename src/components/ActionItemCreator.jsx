...
import { Mic, Square, Download, Upload, Trash2, CheckCircle, Edit2, AlertCircle, FileText, Globe, Volume2 } from 'lucide-react';
...
const useSpeechRecognition = (language, onTranscriptUpdate, onError) => {
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognition.onend = () => {
        // Auto-restart logic handled by parent component
      };
...
[
  {
    ...
    try {
      ...
      const validatedItems = parsedItems.map((item, index) => {
        if (!item.actionName || !item.actionBy || !item.dueDate || !Array.isArray(item.process)) {
          console.warn(`Invalid action item at index ${index}:`, item);
          return {
            actionName: item.actionName || 'Unnamed Action',
            actionBy: item.actionBy || 'Not Assigned',
            dueDate: item.dueDate || 'TBD',
            process: Array.isArray(item.process) ? item.process : ['Complete task'],
            remarks: item.remarks || ''
          };
        }
        return {
          actionName: sanitizeText(item.actionName),
          actionBy: sanitizeText(item.actionBy),
          dueDate: sanitizeText(item.dueDate),
          process: item.process.map(step => sanitizeText(step)),
      ...
      try {
        ...
        const validatedItems = data.actionItems.map(item => ({
          ...
          process: Array.isArray(item.process) 
            ? item.process.map(step => sanitizeText(step))
            : ['Complete task'],
          remarks: sanitizeText(item.remarks || '')
        }));
        ...
        if (data.transcript) {
          setTranscript(sanitizeText(data.transcript));
        }
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              ...
              <div className="flex items-center gap-3 bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2">
                ...
                <select
                  ...
                  className="bg-transparent text-white font-semibold border-none outline-none cursor-pointer disabled:cursor-not-allowed disabled:opacity-50"
                >
          ...
          <div className="p-6 space-y-6">
            <div className="bg-gradient-to-br from-gray-50 to-blue-50 rounded-xl p-6 border-2 border-gray-200 shadow-sm">
              ...
              <div className="flex gap-3 mb-4 flex-wrap">
                ...
                <button
                  onClick={resetAll}
                  className="flex items-center gap-2 px-6 py-3 bg-gray-500 hover:bg-gray-600 text-white rounded-lg font-semibold transition-all shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              <textarea
                ...
                placeholder={`Your ${selectedLanguage === LANGUAGES.HINDI.code ? 'Hindi' : 'English'} transcript will appear here...\n\nOr type/paste text directly. Supported content:\n• Meeting discussions\n• Task assignments\n• Project planning notes\n• Action items and deadlines`}
                className="w-full h-64 p-4 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none font-mono text-sm bg-white shadow-inner disabled:bg-gray-100 disabled:cursor-not-allowed"
              ...
              <div className="mt-4 flex items-start gap-2 text-sm text-gray-600 bg-blue-50 p-3 rounded-lg border border-blue-200">
                ...
                <div>
                  ...
                  <ul className="list-disc list-inside space-y-1 mt-1">
                    <li>Speak clearly about tasks, assignments, responsibilities, and deadlines</li>
                    <li>Mention names, dates, and specific action items</li>
                    <li>Use natural language - AI will extract structured action items</li>
            ...
            <div className="text-center">
              <button
                ...
                className="px-12 py-4 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white rounded-xl font-bold text-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-xl transform hover:scale-105"
              >
                {isProcessing ? (
                  <span className="flex items-center gap-3">
                    <div className="w-5 h-5 border-3 border-white border-t-transparent rounded-full animate-spin"></div>
                    Analyzing Transcript & Extracting Action Items...
                  </span>
                ) : (
                ...
                <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                      ...
                      {isFrozen && <span className="ml-2 text-green-600 font-semibold">✓ Frozen (Read-Only)</span>}
                    </p>
